...
export default function LoanCalculator() {
    // Set default values from your screenshot
    ...
    const [principalComponents, setPrincipalComponents] = useState([
        { id: 1, type: 'Tuition', amount: '54000' },
        { id: 2, type: 'Upkeep', amount: '69000' },
        { id: 3, type: 'GFF Subscription', amount: '3000' },
        { id: 4, type: 'Hospital Cash', amount: '2000' },
        { id: 5, type: 'Insurance', amount: '2000' }
    ]);
    ...
    const generatePDF = () => {
        ...
        const summaryData = [
            ...
            ['REMAINING PRINCIPAL BALANCE', `${calculations.remainingAfterUpfront.toLocaleString()}`]
        ];
        ...
        const termsData = [
            ...
            ['Required Weekly Payment', `${calculations.requiredWeeklyPayment.toFixed(2)} KES`],
            ['Extra Weekly Prepayment', `${calculations.extraWeeklyPrepayment.toFixed(2)} KES`],
        ...
        doc.text(
            `Extra weekly payment of ${calculations.extraWeeklyPrepayment.toFixed(2)} KES applied as prepayment to principal.`,
            105,
            finalY + 10,
            { align: 'center' }
        );
    ...
    const calculations = useMemo(() => {
        ...
        const remainingAfterUpfront = remainingGfkFee + remainingCollateral + remainingInterest + remainingPrincipalComponents;

        // Generate payment schedule - NO FIXED REQUIRED PAYMENT CALCULATION
        const schedule = [];
        let currentBalance = remainingAfterUpfront;
        let currentPriority = 1;
        let totalInterestPaid = 0;

        // Set first payment date (Oct 28, 2025 as in your image)
        const firstPaymentDate = new Date(2025, 9, 28); // Month is 0-indexed (9 = October)

        let week = 1;
        while (currentBalance > 0.01 && week <= 500) {
            // Calculate interest for this period
            const interestPayment = currentBalance * weeklyInterestRate;
            totalInterestPaid += interestPayment;

            let paymentRemaining = weeklyPayment;
            ...
            // 1. Pay current week's interest first
            if (paymentRemaining > 0 && interestPayment > 0) {
            ...
            if (paymentRemaining > 0) {
                ...
                if (remainingGfkFee > 0.01) {
                    gfkFeePayment = Math.min(paymentRemaining, remainingGfkFee);
                    remainingGfkFee -= gfkFeePayment;
                    paymentRemaining -= gfkFeePayment;
                    actualPrincipalPayment += gfkFeePayment;
                    currentPriority = 1;
                }

                // PRIORITY 2: Collateral Fee (only if Priority 1 is paid off)
                if (paymentRemaining > 0 && remainingGfkFee <= 0.01 && remainingCollateral > 0.01) {
                    collateralPayment = Math.min(paymentRemaining, remainingCollateral);
                    remainingCollateral -= collateralPayment;
                    paymentRemaining -= collateralPayment;
                    actualPrincipalPayment += collateralPayment;
                    currentPriority = 2;
                }

                // PRIORITY 3: Outstanding Interest (only if Priorities 1 & 2 are paid off)
                if (paymentRemaining > 0 && remainingGfkFee <= 0.01 && remainingCollateral <= 0.01 && remainingInterest > 0.01) {
                    interestPrincipalPayment = Math.min(paymentRemaining, remainingInterest);
                    remainingInterest -= interestPrincipalPayment;
                    paymentRemaining -= interestPrincipalPayment;
                    actualPrincipalPayment += interestPrincipalPayment;
                    currentPriority = 3;
                }

                // PRIORITY 4: Principal Components (only if Priorities 1, 2 & 3 are paid off)
                if (paymentRemaining > 0 && remainingGfkFee <= 0.01 && remainingCollateral <= 0.01 && remainingInterest <= 0.01 && remainingPrincipalComponents > 0.01) {
                    principalComponentsPayment = Math.min(paymentRemaining, remainingPrincipalComponents);
                    remainingPrincipalComponents -= principalComponentsPayment;
                    paymentRemaining -= principalComponentsPayment;
                    actualPrincipalPayment += principalComponentsPayment;
                    currentPriority = 4;
                }

                // Any remaining payment goes as extra prepayment to principal
                if (paymentRemaining > 0.01) {
                    extraPrepayment = paymentRemaining;
                    remainingPrincipalComponents = Math.max(0, remainingPrincipalComponents - extraPrepayment);
                    actualPrincipalPayment += extraPrepayment;
                    paymentRemaining = 0;
                }
            }

            currentBalance = remainingGfkFee + remainingCollateral + remainingInterest + remainingPrincipalComponents;

            // Calculate payment date
            const paymentDate = new Date(firstPaymentDate);
            paymentDate.setDate(firstPaymentDate.getDate() + (week - 1) * 7);
            ...
            schedule.push({
                ...
                extraPrepayment: extraPrepayment,
                remainingPrincipal: currentBalance,
                // Track individual component payments
                gfkFeePayment,
                collateralPayment,
        ...
        // Calculate what the required weekly payment would have been for comparison
        const calculateRequiredPayment = (principal, weeklyRate, weeks) => {
            return principal * weeklyRate / (1 - Math.pow(1 + weeklyRate, -weeks));
        };

        const requiredWeeklyPayment = calculateRequiredPayment(remainingAfterUpfront, weeklyInterestRate, schedule.length);
        const extraWeeklyPrepayment = weeklyPayment - requiredWeeklyPayment;

        // Calculate effective annual rate for verification
        const effectiveAnnualRate = (Math.pow(1 + weeklyInterestRate, 52) - 1) * 100;
                            ...
                            <div className="grid md:grid-cols-3 gap-6">
                                <div className="bg-white rounded-lg p-4 shadow-sm border border-green-300">
                                    ...
                                    <div className="text-xs text-gray-500 mt-1">
                                        Fixed weekly amount paid by user
                                    </div>
                                </div>
                                <div className="bg-white rounded-lg p-4 shadow-sm border border-blue-300">
                                    <div className="text-sm text-gray-600 mb-1">Required Payment</div>
                                    <div className="text-2xl font-bold text-blue-600">
                                        {calculations.requiredWeeklyPayment.toFixed(2)} KES
                                    </div>
                                    <div className="text-xs text-gray-500 mt-1">
                                        Calculated for exact 20% rate
                                    </div>
                                </div>
                                <div className="bg-white rounded-lg p-4 shadow-sm border border-purple-300">
                                    <div className="text-sm text-gray-600 mb-1">Extra Prepayment</div>
                                    <div className="text-2xl font-bold text-purple-600">
                                        {calculations.extraWeeklyPrepayment.toFixed(2)} KES
                                    </div>
                                    <div className="text-xs text-gray-500 mt-1">
                                        Applied to principal weekly
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-6 mb-8 border-2 border-blue-200">
                            ...
                            <div className="mt-6 grid md:grid-cols-2 gap-6">
                                ...
                                <div className="bg-white rounded-lg p-4 shadow-sm border-2 border-green-300">
                                    ...
                                    <div className="text-2xl font-bold text-green-600">
                                        {calculations.remainingAfterUpfront.toLocaleString()} KES
                                    </div>
                                    <div className="text-xs text-gray-500 mt-1">
                        ...
                        <div className="bg-gray-50 rounded-lg p-6">
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    ...
                                    <tr className="bg-gray-200 text-gray-700">
                                        ...
                                        <th className="px-3 py-3 text-right font-semibold">Extra Prepay</th>
                                    ...
                                    <tbody>
                                    {calculations.schedule.slice(0, 20).map((row, idx) => (
                                        <tr
                                            key={idx}
                                            className={`border-b border-gray-200 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}
                                        >
                                            <td className="px-3 py-2 font-medium text-black">{row.week}</td>
                                            <td className="px-3 py-2 text-black">{row.date}</td>
                          ...
                          <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${
                            ...
                            {row.currentPriority}
                                ...
                                {calculations.schedule.length > 20 && (
                                    <div className="mt-4 text-center text-gray-600">
                                        Showing first 20 of {calculations.schedule.length} weeks. Download PDF for complete schedule.
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="mt-4 flex items-start gap-2 text-sm text-gray-600 bg-blue-50 p-4 rounded-lg">
                                ...
                                <div>
                                    ...
                                    <strong>Extra Payment:</strong> {calculations.extraWeeklyPrepayment.toFixed(2)} KES weekly applied as prepayment to principal<br />
