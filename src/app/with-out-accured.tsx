...
export default function LoanCalculator() {
    const [isYearOne, setIsYearOne] = useState(true);
    ...
    // Items that can be in either category depending on year
    const rolloverItems = ['Insurance', 'Hospital Cash', 'GFF Subscription'];
    ...
    const generatePDF = () => {
        ...
        doc.text(`Loan Year: ${isYearOne ? 'Year 1' : 'After Year 1'}`, 105, yPos + 5, { align: 'center' });
        ...
        const gfkTableData = calculations.effectiveGfkComponents
        ...
        const principalTableData = calculations.effectivePrincipalComponents
            .filter(c => c.type || c.amount)
            .map(c => [c.type || 'N/A', `${parseFloat(c.amount || 0).toLocaleString()} KES`]);
        ...
        doc.text(
            `(3) Outstanding Interest (accrues weekly at ${interestRate}% p.a.), (4) Principal.`,
            105,
            finalY + 5,
            { align: 'center' }
        );
        doc.text(
            isYearOne ? 'Year 1: Insurance, Hospital Cash, and GFF Subscription are rolled into Principal.' : 'After Year 1: Insurance, Hospital Cash, and GFF Subscription are in GFK components.',
            105,
            finalY + 10,
            { align: 'center' }
        );
    ...
    const calculations = useMemo(() => {
        // Determine which components go where based on year
        let effectiveGfkComponents = [...gfkComponents];
        let effectivePrincipalComponents = [...principalComponents];

        if (isYearOne) {
            // In Year 1: Move Insurance, Hospital Cash, GFF Subscription from GFK to Principal
            effectiveGfkComponents = gfkComponents.filter(c =>
                !rolloverItems.includes(c.type)
            );

            // Add rollover items from GFK to Principal
            const rolloverFromGfk = gfkComponents.filter(c =>
                rolloverItems.includes(c.type) && (c.amount && parseFloat(c.amount) > 0)
            );
            effectivePrincipalComponents = [...principalComponents, ...rolloverFromGfk];
        }

        // Calculate totals
        const totalGfk = effectiveGfkComponents.reduce((sum, c) => sum + (parseFloat(c.amount) || 0), 0);
        const totalPrincipal = effectivePrincipalComponents.reduce((sum, c) => sum + (parseFloat(c.amount) || 0), 0);
        ...
        // Calculate fees on TOTAL (GFK + Principal)
        ...
        while (remainingGfkFee > 0.01 || remainingCollateral > 0.01 || remainingInterest > 0.01 || remainingPrincipal > 0.01) {
            ...
            // Add weekly interest to outstanding balance ONLY if principal is being paid
            ...
            if (payment > 0 && remainingPrincipal > 0) {
                ...
                // Only accrue interest if we're actually paying principal
            ...
            schedule.push({
                ...
                interestPayment,
                principalPayment,
                weeklyInterestAccrued: principalPayment > 0 ? weeklyInterestAccrued : 0,
                remainingGfkFee,
                remainingCollateral,
                remainingInterest,
                remainingPrincipal
        ...
        return {
            ...
            effectiveGfkComponents,
            effectivePrincipalComponents
        };
    }, [gfkComponents, principalComponents, upfrontAmount, outstandingInterest, interestRate, isYearOne]);
                ...
                <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    <div className="p-8">
                        ...
                        <div className="mb-8 bg-yellow-50 border-2 border-yellow-300 rounded-lg p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    <h3 className="font-bold text-gray-800 mb-1">Loan Year</h3>
                                    <p className="text-sm text-gray-600">
                                        {isYearOne
                                            ? 'Year 1: Insurance, Hospital Cash & GFF Subscription are rolled into Principal'
                                            : 'After Year 1: Insurance, Hospital Cash & GFF Subscription are GFK components'}
                                    </p>
                                </div>
                                <div className="flex gap-2">
                                    <button
                                        onClick={() => setIsYearOne(true)}
                        ...
                        <div className="grid lg:grid-cols-2 gap-8 mb-8">
                            ...
                            <div className="bg-purple-50 rounded-lg p-6 border-2 border-purple-200">
                                <h3 className="text-lg font-bold text-gray-800 mb-2">GFK Components</h3>
                                <p className="text-xs text-gray-600 mb-4">
                                    {isYearOne && '(Note: Insurance, Hospital Cash, GFF Subscription will be moved to Principal in Year 1)'}
                                </p>
                                {gfkComponents.map((component, idx) => (
                                    <div key={component.id} className="flex gap-2 mb-3">
                                        <input
                                            ...
                                            className="flex-1 px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                                ...
                                <div className="mt-4 p-3 bg-white rounded border border-purple-300">
                                    <div className="text-sm text-gray-600">Effective GFK Amount:</div>
                            ...
                            <div className="bg-blue-50 rounded-lg p-6 border-2 border-blue-200">
                                <h3 className="text-lg font-bold text-gray-800 mb-2">Principal Components</h3>
                                <p className="text-xs text-gray-600 mb-4">
                                    {isYearOne && '(Insurance, Hospital Cash, GFF Subscription from GFK will be added here)'}
                                </p>
                                ...
                                <div className="mt-4 p-3 bg-white rounded border border-blue-300">
                                    <div className="text-sm text-gray-600">Effective Principal Amount:</div>
                        ...
                        <div className="bg-gray-50 rounded-lg p-6">
                            ...
                            <div className="mt-4 flex items-start gap-2 text-sm text-gray-600 bg-blue-50 p-4 rounded-lg">
                                ...
                                <div>
                                    ...
                                    (1) GFK Fee (GFK components + 5% GFF fee), (2) Collateral (10% fee), (3) Outstanding Interest (accrues weekly at {interestRate}% p.a. only when principal is being paid), (4) Principal.
                                    Upfront amount is applied to these categories in the same order before payments begin.
                                    {isYearOne && ' Note: In Year 1, Insurance, Hospital Cash, and GFF Subscription are rolled into Principal.'}
                                </div>
                            </div>
                        </div>
                    </div>
